...
const PostFooter = (props) => {
  ...
  const [initLikesData,setInitLikesData] = useState({})
  const [isLikedColor,setIsLikedColor] = useState("lightblue")
  const [likeCount,setLikeCount] = useState (props.participant.likes)
  ...
  useEffect(() => { 
    ...
    const isLiked = async()=> {
      try {
        ...
        .then(res => setInitLikesData({...res.data}) )
      } catch (error) {
        console.log(error)
      }
    }
  ...
  useEffect(() => {
   console.log(initLikesData)
  initLikesData.isliked ?  setIsLikedColor("blue") : setIsLikedColor("lightblue")
  setLikeCount(initLikesData
  )

  },[initLikesData] )

  useEffect(() => {
   likeCount.isliked ?  setIsLikedColor("blue") : setIsLikedColor("lightblue")
  ...
  return (
    ...
    <div className='d-flex align-items-center gap-3'>
       <button style={{color:isLikedColor,marginLeft:'15px'}} onClick={handleLikes} >
      ...
      <span style={{fontSize:'11px',marginLeft:'0px'}}>{likeCount.like_count}</span> 
    </div>
    <div className='d-flex align-items-center gap-3'>
       ...
       <span style={{fontSize:'12px',color:'pink'}}>Votes</span> 
       <span style={{fontSize:'11px',marginLeft:'0px'}}>{props.participant.votes}</span> 
       </div>
